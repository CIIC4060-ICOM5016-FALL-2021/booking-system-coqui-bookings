...
class BookingDAO:
    ...
    def updateBooking(self, booking_id, booking_name, booking_time_start, booking_time_end, user_id, room_id):
     ...
     # Update
    def updateBookingName(self, booking_id, booking_name, user_id):
        cursor = self.conn.cursor()
        query = 'update "Booking" set booking_name = %s, user_id = %s,' \
                'where booking_id = %s'
        cursor.execute(query, (booking_name, user_id, booking_id))
        self.conn.commit()
        return True
